...
export const Alphabetical = ({ items }: innerProps) => {
  ...
  const { isSensitiveContentEnabled, isGrid } = useStore();
  ...
  const mappedList = useMemo(() => {
    ...
    [...items, ...customSounds]
      ...
      .filter(
        (item) =>
          ...
          item.image.includes(search)
      )
      .forEach((item) => {
        const index = isGrid ? '' : item.name.charAt(0).toLowerCase();
        if (list[index]) {
    ...
    return list;
  }, [customSounds, isSensitiveContentEnabled, items, search, isGrid]);
  ...
  return (
    ...
    >
      ...
      >
        {Object.keys(mappedList)
          .sort()
          .map((k) => (
            <View key={k}>
              <Text
                style={{
                  ...tailwind('text-3xl pt-2 px-4 font-extrabold'),
